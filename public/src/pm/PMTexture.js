export default class PMTexture {
    ...
    loadTexture(objDin, fun) {
        ...
        if (!this.textureCache[objDin.id].texture) {
           ...
           //  path.pop();
            ...
            const texture = this.loader.load(mhbd.getLink(link), texture => {
    ...
    getObj(id) {  
        ...
        var oPar=this.param[this.nameType]
        if(mhbd.objectBase && mhbd.objectBase["textures"])oPar=mhbd.objectBase["textures"]
        ...
        for (var i = 0; i < oPar.length; i++) {
            ...
            if(id==oPar[i].id){
                ...
                let o
                if(typeof oPar[i].json === "string") {
                    o = JSON.parse(oPar[i].json)             
                } else o = oPar[i].json;
