...
export class TVectorDebag {
	constructor(tv) {
		...
		var ggg
		this.input=new DInput(this.dCont,204,500,"a",function(){
			
			var bmp=tv.bitmapData;
			var ctx=bmp.ctx;

			if(bmp.width!=200)bmp.width=200;			
			if(bmp.height!=200)bmp.height=200;
			...
			ctx.fillText(this.text, 0, 175);
			bmp.imgData = ctx.getImageData(0, 0, 200, 200);
			//tv.bitmapData.upDate();
			tv.upDate();
			
			if(ggg==undefined){
				ggg=new GeometriSimvol(tv.points.arrayTriangle,tv.points.array);
				var m=new THREE.Mesh(ggg);
				o3d.add(m);
			}else{
				ggg.serAA(tv.points.arrayTriangle,tv.points.array);
			}
...
function GeometriSimvol(arr,arr2) {
...
GeometriSimvol.prototype = Object.create(THREE.BufferGeometry.prototype);
GeometriSimvol.prototype.constructor = GeometriSimvol;
