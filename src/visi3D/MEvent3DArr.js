...
export default function MEvent3DArr (par, camera, contentHTML, stage) {
	...
	this.mouseRay = function () {
		...
		if (this.boolNum == -1) {
			if (this.uuid != 'nullMy') {
				self.dragAllE3d('out', this.event3D)
			...
			if (this.uuid != 'nullMy') { // обьект есть
				...
				if (this.uuid != this.intersects[this.boolNum].object.uuid) { // это новый обьект
					...
					self.dragAllE3d('over', this.event3D)
					...
					this.event3D.uv = this.intersects[this.boolNum].uv;
					this.event3D.originalEvent = self.event;
					self.dragAllE3d('move', this.event3D)
	...
	this.restartPoint = function (e) {
		if(e && e.target){
			rect = self.par.contentHTML.getBoundingClientRect();
	...
	this.mouseup = function (e) {
		...
		if (self.devas==false){	
			if (self.boolNumOld == -1) {
				...
				self.dragAllE3d('up', self.event3D)
	...
	this.mousedown = function (e) {
		...
		if (self.devas==false){	
			if (self.boolNumOld == -1) {
				...
				self.event3D.target = null;
				self.event3D.originalEvent = self.event;
				self.dragAllE3d('down', self.event3D)
...
s
	...
	this.mousewheel = function (e) {
		...
		if (self._activ == false) return;
		if (self.boolNumOld == -1) {
			self.dragAllE3d('down', null)
			self.eventSob.dispatcherEvent('down', null);
		} else {
			self.event3D.originalEvent = self.event;
			self.dragAllE3d('wheel', self.event3D)
			self.eventSob.dispatcherEvent('wheel', self.event3D);
		}
	};
	...
	this.dragE3d=undefined
	this.dragAllE3d=function(s,e3d){
	
		if(self.dragE3d!==undefined)self.dragE3d(s,e3d)
    } 
